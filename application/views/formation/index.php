<style>
    th{
        text-align: center;
    }
</style>

<?php
    if(($this->session->doc_upload_failed))
    {
?>
        <script>
            Swal.fire({            
            icon: 'warning',
            title: 'Vos fichier n\'ont pas été enregistrés!! veuillez ressayer de les fournir en cliquant sur ~Gerer mon profile~',
            showConfirmButton: true,
            timer: 30000
            })
        </script>
<?php
    }
    if(($this->session->formation_chosen))
    {
?>
        <script>
            Swal.fire({            
            icon: 'success',
            title: 'Transfusion Supprimé',
            showConfirmButton: false,
            timer: 3000
            })
        </script>
<?php
    }
?>

<section class="content">
    <header class="content__title">
        <h1 class="animated"><b>Formations</b></h1>
    </header>

    <div class="card animated fadeIn">
        <div class="card-body">
        <header class="content__title">
            <div class="row">
                <div class="col-md-9">
                    <p><b>Voici la liste de toutes les formations disponibles pour tout type de candidats</b></p>
                </div>
                <?php
                    if($this->session->type == 'admin')
                    {
                ?>
                    <div class="col-md-3 offset-md-3">
                        <a href="<?=site_url('formation/new_formation')?>" class="btn btn-secondary"><i class="zmdi zmdi-plus zmdi-hc-fw"></i> Nouvelle Formation</a>
                    </div>
                <?php
                    }
                ?>
            </div>
        </header>
            <div class="table-responsive">
                <table id="data-table" class="table table-bordered table-hover">
                    <thead class="thead-default">
                        <tr class="alert alert-info">
                            <th style="width: 20px;">No</th>
                            <th>Intitulé</th>                         
                            <th>Branche</th>
                            <th>Durée</th>
                           <th>Tarif</th>
                            <!--<th>Quantite</th> -->
                            <?php if($this->session->connected){
                                ?>
                                <th style="width: 180px;">Actions</th>
                            <?php
                                }
                            ?>
                        </tr>
                    </thead>                    
                    <tbody id="t-body">
                        <?php
                            $num = 0;
                            foreach($formation as $f)
                            {
                                ++$num;
                        ?>                                
                                <tr>
                                    <td style="text-align: center;"><?=$num?></td>
                                    <td style="text-align: center;"><?=$f->intitule?></td>                  
                                    <td style="text-align: center;"><?=$f->branche?></td>
                                    <td style="text-align: center;"><?=$f->duree?></td>
                                    <td style="text-align: center;"><?=$f->tarif?> $</td>
                                    <?php
                                    if($this->session->connected)
                                    {
                                    ?>
                                        <td class="text-center">
                                            <a href="<?=site_url('payment/new_paiement?formation_id='.$f->id)?>"
                                                <button class="btn btn-danger btn--raised">Choisir</button>  
                                            </a>                                                                                                           
                                        </td>
                                    <?php
                                    }
                                    ?>                                    
                                </tr>
                        <?php
                            }
                        ?>
                       
                    </tbody>
                </table>
            </div>
        </div>
    </div>
</section>

